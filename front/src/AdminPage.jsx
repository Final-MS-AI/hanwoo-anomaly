...
function AdminPage() {
  ...
  const loadTracks = async () => {
    ...
    try {
      ...
      setFeedback((payload.tracks || []).map(trackToFeedback));
    } catch {
  ...
  useEffect(() => {
    if (permission === "granted") loadTracks();
  }, [permission]);
    ...
    <main className="admin-shell">
      <aside className="admin-sidebar">
        <button className="admin-brand" type="button" onClick={() => navigate("/dashboard")}>
          ...
          <span>COWOW <small>관리자</small></span>
        </button>
        ...
        <nav className="admin-nav" aria-label="관리자 메뉴">
          {navItems.map(([key, icon, label]) => (
            <button key={key} className={activeNav === key ? "active" : ""} type="button" onClick={() => setActiveNav(key)}>
              <span>{icon}</span>{label}{key === "feedback" && <b>12</b>}
            </button>
          ))}
        </nav>
      ...
      <section className="admin-content">
        <header className="admin-topbar">
          ...
          <div className="admin-top-actions"><span className="live-pill"><span /> 운영 중</span><button className="icon-button" type="button" aria-label="알림">♧<em>3</em></button><div className="admin-avatar">SY</div><strong>관리자</strong></div>
        </header>

        <div className="admin-main">
          <div className="admin-heading-row"><div><p className="eyebrow">2026년 8월 21일 목요일</p><h1>{activeNav === "overview" ? "관리자님, 좋은 아침이에요" : navItems.find(([key]) => key === activeNav)?.[2]} <span>✦</span></h1><p className="admin-subtitle">{activeNav === "overview" ? "오늘의 모델 상태와 피드백 흐름을 한눈에 확인하세요." : "비문 식별과 ID 역전파 운영 현황을 관리하세요."}</p></div><button className="primary-button" type="button" onClick={() => setNotice("최신 데이터로 새로고침했습니다.")}>↻ 데이터 새로고침</button></div>

          {notice && <div className="admin-toast" role="status">✓ {notice}</div>}

          {activeNav === "overview" && <OverviewContent tracks={feedback} filteredFeedback={filteredFeedback} filter={filter} setFilter={setFilter} setActiveNav={setActiveNav} reviewItem={reviewItem} trackLoading={trackLoading} />}
          {activeNav === "feedback" && <FeedbackWorkspace filteredFeedback={filteredFeedback} filter={filter} setFilter={setFilter} reviewItem={reviewItem} onBound={loadTracks} trackLoading={trackLoading} trackError={trackError} />}
          {activeNav === "loop" && <LoopWorkspace setNotice={setNotice} />}
...
function AdminGate({ title, detail }) {
  const navigate = useNavigate();
  return <main className="admin-gate"><div className="admin-gate-card"><span className="admin-brand-mark">C</span><h1>{title}</h1><p>{detail}</p><button className="primary-button" type="button" onClick={() => navigate("/dashboard")}>대시보드로 돌아가기</button></div></main>;
}

function OverviewContent({ tracks, filteredFeedback, filter, setFilter, setActiveNav, reviewItem, trackLoading }) {
  ...
  return <>
    ...
    <div className="admin-grid-main">
      <section className="admin-panel feedback-panel"><div className="panel-heading"><div><h2>ID 역전파 검토함</h2><p>비문 식별 결과를 트랙에 연결하기 전에 확인하세요.</p></div><button className="text-button" type="button" onClick={() => setActiveNav("feedback")}>전체 보기 →</button></div><FeedbackFilters filter={filter} setFilter={setFilter} count={filteredFeedback.length} /><div className="feedback-list">{filteredFeedback.map((item) => <FeedbackRow key={item.id} item={item} onReview={reviewItem} />)}</div></section>
      <LoopCard setNotice={setActiveNav} />
    </div>
    <ActivityPanel />
  </>;
}
...
function FeedbackWorkspace({ filteredFeedback, filter, setFilter, reviewItem, onBound, trackLoading, trackError }) {
  ...
  return <><section className="admin-panel full-panel feedback-workspace"><div className="panel-heading"><div><h2>전체 ID 역전파 검토</h2><p>muzzle API에서 불러온 실제 트랙과 현재 바인딩 상태입니다.</p></div><span className="count-badge">검토 대기 {filteredFeedback.filter((item) => item.status === "검토 대기").length}건</span></div>{trackError && <div className="api-error" role="alert">⚠ {trackError}</div>}<FeedbackFilters filter={filter} setFilter={setFilter} count={filteredFeedback.length} />{trackLoading ? <div className="empty-state">muzzle 트랙을 불러오는 중입니다…</div> : filteredFeedback.length === 0 ? <div className="empty-state">선택한 조건에 해당하는 트랙이 없습니다.</div> : <div className="feedback-list expanded-feedback-list">{filteredFeedback.map((item) => <FeedbackRow key={item.id} item={item} onReview={reviewItem} onSelect={setSelectedSegment} expanded />)}</div>}</section>{selectedSegment && <TrackDetailPanel segmentId={selectedSegment} onClose={() => setSelectedSegment(null)} onChanged={onBound} />}<BindingForm onBound={onBound} /></>;
}
...
function TrackDetailPanel({ segmentId, onClose, onChanged }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`${MUZZLE_API}/tracks/${segmentId}`, { credentials: "include" }).then(async (response) => {
      const payload = await response.json();
      if (!response.ok) throw new Error(payload?.detail || `HTTP ${response.status}`);
      if (!cancelled) { setTrack(payload); setState({ type: "", message: "" }); }
    }).catch((error) => { if (!cancelled) setState({ type: "error", message: `상세 조회 실패: ${error.message}` }); });
...
function BindingForm({ onBound }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setSegmentId(""); setNationalId("");
      onBound();
    } catch (error) {
      setState({ type: "error", message: `바인딩 실패: ${error.message}` });
    }
  };
  return <section className="admin-panel full-panel binding-form-panel"><div className="panel-heading"><div><h2>트랙에 ID 직접 바인딩</h2><p>muzzle API의 POST /tracks/{"{segment_id}"}/bind를 호출합니다. 유사도 0.70 미만은 서버에서 거부됩니다.</p></div><span className="api-connected-badge">실제 API</span></div><form className="binding-form" onSubmit={submit}><label>segment ID<input inputMode="numeric" value={segmentId} onChange={(event) => setSegmentId(event.target.value)} placeholder="예: 184" required /></label><label>가축이력번호<input value={nationalId} onChange={(event) => setNationalId(event.target.value)} placeholder="예: 410012345678" required /></label><label>유사도<input type="number" min="0.70" max="1" step="0.0001" value={similarity} onChange={(event) => setSimilarity(event.target.value)} required /></label><button className="primary-button" type="submit" disabled={state.type === "loading"}>{state.type === "loading" ? "처리 중…" : "ID 바인딩 적용"}</button></form>{state.message && <p className={`binding-message ${state.type}`} role="status">{state.message}</p>}</section>;
}
...
function LoopWorkspace({ setNotice }) {
  return <><section className="admin-panel full-panel loop-detail-panel"><div className="panel-heading"><div><h2>비문 ID 역전파 흐름</h2><p>한 번의 바인딩이 트랙의 과거 관측 전체에 반영되는 과정을 확인합니다.</p></div><span className="running-badge"><span /> 진행 중</span></div><div className="large-loop"><div className="large-loop-step done"><b>✓</b><strong>비문 식별</strong><small>코무늬 임베딩과 유사도 비교</small></div><div className="large-loop-step done"><b>✓</b><strong>트랙 연결</strong><small>track_segment에 개체 바인딩</small></div><div className="large-loop-step current"><b>03</b><strong>ID 역전파</strong><small>과거 track_observation에 소급</small></div><div className="large-loop-step"><b>04</b><strong>타임라인 확인</strong><small>개체별 이력으로 조회</small></div></div></section><section className="admin-panel full-panel"><div className="panel-heading"><div><h2>최근 바인딩 작업</h2><p>muzzle API를 통해 처리된 트랙 바인딩입니다.</p></div><button className="secondary-button compact-button" type="button" onClick={() => setNotice("트랙 상세 화면은 muzzle API 연결 후 제공됩니다.")}>트랙 API 확인</button></div><div className="binding-table"><div className="binding-head"><span>트랙</span><span>개체 ID</span><span>유사도</span><span>소급 관측</span><span>상태</span></div><div><span>segment #184</span><strong>410012345678</strong><span>0.8421</span><span>248건</span><em>적용 완료</em></div><div><span>segment #179</span><strong>410012345612</strong><span>0.7814</span><span>126건</span><em>적용 완료</em></div><div><span>segment #188</span><strong>미확정</strong><span>0.6632</span><span>—</span><em className="waiting">검토 대기</em></div></div></section></>;
}
...
function LoopCard({ setNotice }) {
  return <section className="admin-panel loop-panel"><div className="panel-heading"><div><h2>비문 ID 역전파 흐름</h2><p>확정된 개체 ID가 트랙의 과거 관측에 소급 적용됩니다.</p></div><span className="running-badge"><span /> 진행 중</span></div><div className="loop-visual"><div className="loop-line"><span className="loop-progress" /></div>{[["01", "비문 식별", "코무늬 유사도 판정", "done"], ["02", "트랙 연결", "개체 ID 바인딩", "done"], ["03", "ID 역전파", "과거 관측에 소급", "current"], ["04", "타임라인", "개체 이력 확인", "pending"]].map(([number, title, desc, state]) => <div className={`loop-step ${state}`} key={number}><span className="step-number">{state === "done" ? "✓" : number}</span><strong>{title}</strong><small>{desc}</small></div>)}</div><div className="loop-footer"><div><span>현재 트랙</span><strong>segment #184</strong></div><div><span>소급 반영 관측</span><strong>248건</strong></div><div><span>운영 임계값</span><strong>유사도 0.70</strong></div></div><button className="secondary-button" type="button" onClick={() => setNotice("loop")}>트랙 상세 보기 →</button></section>;
}

function ActivityPanel() { return <section className="admin-panel activity-panel"><div className="panel-heading"><div><h2>최근 활동</h2><p>최근 ID 바인딩과 비문 식별 시스템 이벤트입니다.</p></div><button className="text-button" type="button">활동 로그 →</button></div><div className="activity-list"><Activity icon="✓" tone="green" title="트랙 172 ID 바인딩 완료" detail="한우 410012345612에 연결하고 과거 관측 126건에 반영했습니다." time="오늘 09:42" /><Activity icon="⟳" tone="purple" title="비문 영상 식별 완료" detail="초크포인트 영상 3건에서 확정 후보를 계산했습니다." time="오늘 03:00" /><Activity icon="!" tone="orange" title="미확정 트랙 증가" detail="유사도 0.70 미달 트랙 2건이 검토 대기 상태입니다." time="어제 18:24" /></div></section>; }
