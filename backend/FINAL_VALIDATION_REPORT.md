...
- Pytest: 45 / 45 PASS, 전체 테스트 3회 연속 PASS
- User QA: 30 / 30 PASS
...
- HTTP 정상: 30 / 30
- Cleanup 정상: 30 / 30
- 문서 범위 밖 일반지식 fallback: 2 / 2
- OOD sources 빈 배열 및 citation 미생성: 2 / 2
- HTTP 오류: 0
...
다음과 같이 검색 문서의 근거가 부족한 질문은 고정 거절문 대신 일반지식으로 답변한다.

- 파이썬 리스트 정렬 방법
- 오늘 서울 날씨

응답에는 현재 등록된 문서에서 직접 확인된 내용이 아니라는 주의 문구를 포함한다. 법률·동물 건강·방역처럼 최신성 또는 전문적 판단이 중요한 분야에는 최신 법령, 수의사, 관계 기관 등의 확인 안내를 추가한다.

일반지식 fallback에는 가짜 문서 citation을 만들지 않으며 `sources`는 빈 배열로 반환한다. 질문 일부만 문서 근거가 있는 혼합 질문은 문서 기반 답변과 일반지식 보충을 구분해 표시한다.
